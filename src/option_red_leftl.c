...
** Last update Fri May 31 03:34:18 2013 alexandre1 nguyen
*/
  ...
  while (my_strcmp(buff, buff2b) != 0)
    {
      ...
      clean_str(buff2b);
      my_printf("? ");
      if ((r = read(0, buff2, SIZE)) <= 0)
	return (1);
      buff2[r] = '\0';
      buff2b = my_strcpy(buff2b, buff2);
      buff2b[r - 1] = '\0';
      if (my_strcmp(buff, buff2b))
	buff3 = my_strcat(buff3, buff2);
    }
  /* buff3[my_strlen(buff3)] = 4; */
  /* buff3[my_strlen(buff3) + 1] = 0; */
...
t_all	*option_red_leftl(char *buf, t_all *all, char **check)
{
  ...
  if (pid == 0)
  ...
  else
    {
      if (wait(0) == -1)
	return (do_nul(all));
      /* printf("IUHDIUHIZUH  1\n"); */
      /* if (dup2(1, pipefd[1]) == -1) */
      /* 	return (do_nul(all)); */
      /* printf("IUHDIUHIZUH  2\n"); */
